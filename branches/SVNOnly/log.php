...
// Find the first revision of interest
$ignored = 0;
while ($history->entries[$ignored]->rev > $rev)
{
   $ignored++;
}

// Get the number of separate revisions
$revisions = count($history->entries) - $ignored;
...
// Now ask for just the information we want
$history = $svnrep->getLog($path, $history->entries[$firstrevindex + $ignored]->rev, $history->entries[$lastrevindex + $ignored]->rev);
...
foreach ($history->entries as $r)
{
   ...
   if ($dosearch)
   {
      if ((empty($fromRev) || $fromRev > $history->entries[$n]->rev))
      {
   ...
   if ($match)
   {
      ...
      $listing[$index]["revlog"] = nl2br(create_anchors($r->msg));
